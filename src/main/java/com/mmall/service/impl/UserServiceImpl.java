...
//        }
        ServerResponse<String> validResponse = checkValue(user.getUsername(), Const.USERNAME);
        if(!validResponse.isSuccess()){
            return validResponse;
        }
        validResponse = checkValue(user.getEmail(), Const.EMAIL);
        if(!validResponse.isSuccess()){
            return validResponse;
        }
    ...
    public ServerResponse<String> checkValue(String str, String type) {
        ...
        if(Const.EMAIL.equals(type)){
            ...
            if(resultCount > 0){
                return ServerResponse.createByErrorMessage("邮箱已存在已存在");
            }
        }
    ...
    public ServerResponse selectQuestion(String username){
        ServerResponse<String> validResponse = checkValue(username, Const.USERNAME);
        if(validResponse.isSuccess()){
            //用户不存在
